...
def main(email:str, password:str, tries:int, problem_id:str):
    ...
    if success_number > 0:
        analysis_mean /= success_number
        success_number /= tries
        print(f"Mean analysis score for problem {problem_id}: {analysis_mean}")
        with open(f"./SOLVING/{problem_id}/analysis.txt", "a") as f:
            f.write(f"Mean analysis score: {analysis_mean}\n")
            f.write(f"Success rate: {success_number}\n")
