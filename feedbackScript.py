...
def main(email: str, password: str, tries: int, max_reviews: int):
    ...
    prompt_length = 50
    number = random.randint(prompt_length, 100)
    compiler_id = "Python3"
    ...
    for problem_id in problems_id:
        ...
        r = 1
        #if problem_id == "P12284_ca":
        # r = 11
        while r <= max_reviews:

            solutions = prompt_solutions(DENDROGRAM_FILE, number, r)

            for i in range(tries):
                #if (problem_id == "P12284_ca" and r == 11 and i <= 39):
                #    continue
                intento = 0
                while True:
                    try:
                ...
                if not os.path.exists(f"./FEEDBACKS/{problem_id}/Prompt_{r}_solutions/"):
                    os.makedirs(f"./FEEDBACKS/{problem_id}/Prompt_{r}_solutions/", exist_ok=True)
                with open(f"./FEEDBACKS/{problem_id}/Prompt_{r}_solutions/{i}.txt", "w") as f:
                    f.write(answer)
                    print(f"Program saved to ./FEEDBACKS/{problem_id}/Prompt_{r}_solutions/{i}.txt")
                    f.close()
            r += 10
    j.logout()
