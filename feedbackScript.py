...
def decode_solution(problem_id,file_path, number):
    with open(file_path, "r") as f:
        ...
        k = 0
        while (k < number):
            if lines[i + k + 1].strip() and lines[i + k + 1].split()[1] == "Python3":
                k += 1
            else:
                i += 1
...
def ask_for_feedback(client, model, prompt):
        ...
        current_key_index = 0
        TIMERATE = 0
        if os.path.exists("time_log_feedback.txt"):
            with open("time_log_feedback.txt", "r") as f:
                times = f.readlines()
                if len(times) > 0:
                    TIMERATE = sum([float(t.strip()) for t in times]) / len(times)
        while (True):
            try:
                #take current time to calculate total time for the response
                t = time.perf_counter()
                ...
                with open("time_log_feedback.txt", "a") as f:
                    f.write(f"{t}\n")
                    f.close()
...
DENDROGRAM_FILE = "./DENDROGRAMA/DENDROGRAMAS/dendrogram_data.npy"
...
def main(email: str, password: str, tries: int, max_reviews: int):
    ...
    for problem_id in problems_id:
        ...
        proc = subprocess.Popen(["python3", "./dendrogram.py", problem_id, str(max_reviews), path_solutions_relative], cwd="./DENDROGRAMA")
        proc.wait()
        os.chdir(cwd_original)

        r = 1
        if (problem_id == "P67723_ca"):
            r = 41
        while r <= max_reviews:
            number = random.randint(prompt_length, 105)
            solutions = prompt_solutions("./DENDROGRAMA/DENDROGRAMAS/dendrogram_" + problem_id + ".npy", number, r)

            for i in range(tries):
                if (problem_id == "P67723_ca" and r == 41 and i <= 7):
                    continue
                intento = 0
                while True:
                    try:
                ...
                if not os.path.exists(f"./FEEDBACKS/{problem_id}/Prompt_{r}_solutions/"):
                    os.makedirs(f"./FEEDBACKS/{problem_id}/Prompt_{r}_solutions/", exist_ok=True)
                with open(f"./FEEDBACKS/{problem_id}/Prompt_{r}_solutions/{i}_prompt.txt", "w") as f:
                    f.write(prompt)
                    print(f"Prompt saved to ./FEEDBACKS/{problem_id}/Prompt_{r}_solutions/{i}_prompt.txt")
                    f.close()

                # Make a temp file with the feedback or create de directory if it doesn't exist
                if not os.path.exists(f"./FEEDBACKS/{problem_id}/Prompt_{r}_solutions/"):
                    os.makedirs(f"./FEEDBACKS/{problem_id}/Prompt_{r}_solutions/", exist_ok=True)
                with open(f"./FEEDBACKS/{problem_id}/Prompt_{r}_solutions/{i}.md", "w") as f:
                    f.write(answer)
                    print(f"Program saved to ./FEEDBACKS/{problem_id}/Prompt_{r}_solutions/{i}.md")
                    f.close()
            r += 10
    j.logout()
