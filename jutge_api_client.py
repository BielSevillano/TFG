"""
This file has been automatically generated at 2025-02-21T19:21:56.690Z
...
import os
import re
from typing import Any, Type, Optional, BinaryIO, TypeVar
...
BriefProblemDict = dict[str, BriefProblem]
...
HeatmapCalendar = list[DateValue]
...
InstructorListItems = list[InstructorListItem]
...
SubmissionsQuery = list[SubmissionQuery]
...
InstructorEntries = list[InstructorEntry]
...
UsersEmailsAndNames = list[UserEmailAndName]
...
NullableFreeDiskSpaceItem = Optional[FreeDiskSpaceItem]

FreeDiskSpace = dict[str, NullableFreeDiskSpaceItem]
...
SubmissionQueueItems = list[SubmissionQueueItem]
...
UserRanking = list[UserRankingEntry]
...
def _raise_exception(error: dict[str, Any], operation_id: str | None) -> None:
    ...
    raise Exception(f"{name or 'UnknownError'}: {message}")
...
class Util:
    ...
    @staticmethod
    def from_json(s: str, t: Type[T]) -> T:
    ...
    @staticmethod
    def from_yaml(s: str, t: Type[T]) -> T:
...
class JutgeApiClient:
    ...
    def execute(self, func: str, input: Any, ifiles: list[BinaryIO] | None = None) -> tuple[Any, list[Download]]:
        ...
        try:
            response = requests.post(self.JUTGE_API_URL, data={"data": json.dumps(data)}, files=files, timeout=15)
            response.raise_for_status()
        except requests.RequestException as e:
            raise ProtocolException(f"Request error: {e}") from e
        ...
        for part in multipart_data.parts:
            ...
            if b"Content-Type" in part.headers:
                ...
                part_type = get(b"Content-Type")
                ofiles.append(Download(content=part.content, name=filename, type=part_type))
            else:
        ...
        if "error" in answer:
            _raise_exception(answer.get("error", {}), operation_id)
    ...
    def logout(self, silent: bool = False) -> None:
        ...
        try:
            ...
            if not silent:
                print(f"[red]Error at log out: {e}[/red]")
        finally:
            self._meta = None
            if not silent:
...
class ModuleMisc:
    ...
    def get_logo(self) -> Download:
        ...
        if not ofiles:
            ...
            raise ProtocolException("No attached files in response")
